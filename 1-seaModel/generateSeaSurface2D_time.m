function [h, k, S, V, kx, ky,x,y] = generateSeaSurface2D_time(L, N, U10, age, phi_w, w, dt)
%% parameter
% L=4;
...
figure;
%初始化视频
writerObj = VideoWriter('wave_90.avi'); % 指定视频文件名
...
for u = 1:N/2-1
    for v = 1:N/2-1
        ...
        ub = u + 1;
        vb = v + N/2 + 1;
        Va(u,v) = 1/2*sqrt(S(ua,va)*dk^2)*(W1(u,v) + 1i*W2(u,v))*exp(1i*w*t);
        Vb(u,v) = 1/2*sqrt(S(ub,vb)*dk^2)*(W3(u,v) + 1i*W4(u,v))*exp(1i*w*t);
    end
end
...
for j = 2:N/2
    Vx0(j) = 1/2*sqrt(S(1,j)*dk^2)*(w1(j) + 1i*u1(j))*exp(1i*w*t);
    Vy0(j) = 1/2*sqrt(S(j,1)*dk^2)*(w2(j) + 1i*u2(j))*exp(1i*w*t);
end
Vx0(N/2+1) = sqrt(S(1,N/2+1)*dk^2)*u1(1)*exp(1i*w*t);
Vy0(N/2+1) = sqrt(S(N/2+1,1)*dk^2)*u2(1)*exp(1i*w*t);
...
Vx2(1) = sqrt(S(N/2+1,1)*dk^2)*w3(1)*exp(1i*w*t);
Vy2(1) = sqrt(S(1,N/2+1)*dk^2)*w4(1)*exp(1i*w*t);

for j = 2:N/2
    Vx2(j) = 1/2*sqrt(S(N/2+1,j)*dk^2)*(w3(j) + 1i*u3(j))*exp(1i*w*t);
    Vy2(j) = 1/2*sqrt(S(j,N/2+1)*dk^2)*(w4(j) + 1i*u4(j))*exp(1i*w*t);
end

Vx2(N/2+1) = sqrt(S(N/2+1,N/2+1)*dk^2)*u4(1)*exp(1i*w*t);
Vy2(N/2+1) = sqrt(S(N/2+1,N/2+1)*dk^2)*u4(1)*exp(1i*w*t);
