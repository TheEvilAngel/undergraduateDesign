function [h, he, he_t, k, S, Se, V, kx, ky,x,y] = generateSeaSurface2D_time_boat(L, N, U10, age, phi_w, dt, time, vs, beta)
%% parameter
% clear;
% L=4;
...
for u = 1:N/2-1
    for v = 1:N/2-1
        ...
        ub = u + 1;
        vb = v + N/2 + 1;
        Va(u,v) = 1/2*sqrt(S(ua,va)*dk^2)*(W1(u,v) + 1i*W2(u,v))*exp(1i*w(ua,va)*t);
        Vb(u,v) = 1/2*sqrt(S(ub,vb)*dk^2)*(W3(u,v) + 1i*W4(u,v))*exp(1i*w(ub,vb)*t);
        Vae(u,v) = 1/2*sqrt(Se(ua,va)*dk^2)*(W1(u,v) + 1i*W2(u,v))*exp(1i*we(ua,va)*t);
        Vbe(u,v) = 1/2*sqrt(Se(ub,vb)*dk^2)*(W3(u,v) + 1i*W4(u,v))*exp(1i*we(ub,vb)*t);
    end
end
...
for j = 2:N/2
    Vx0(j) = 1/2*sqrt(S(1,j)*dk^2)*(w1(j) + 1i*u1(j))*exp(1i*w(1,j)*t);
    Vy0(j) = 1/2*sqrt(S(j,1)*dk^2)*(w2(j) + 1i*u2(j))*exp(1i*w(j,1)*t);
    
    Vx0e(j) = 1/2*sqrt(Se(1,j)*dk^2)*(w1(j) + 1i*u1(j))*exp(1i*we(1,j)*t);
    Vy0e(j) = 1/2*sqrt(Se(j,1)*dk^2)*(w2(j) + 1i*u2(j))*exp(1i*we(j,1)*t);
end

Vx0(N/2+1) = sqrt(S(1,N/2+1)*dk^2)*u1(1)*exp(1i*w(1,N/2+1)*t);
Vy0(N/2+1) = sqrt(S(N/2+1,1)*dk^2)*u2(1)*exp(1i*w(N/2+1,1)*t);

Vx0e(N/2+1) = sqrt(Se(1,N/2+1)*dk^2)*u1(1)*exp(1i*we(1,N/2+1)*t);
Vy0e(N/2+1) = sqrt(Se(N/2+1,1)*dk^2)*u2(1)*exp(1i*we(N/2+1,1)*t);
...
Vx2(1) = sqrt(S(N/2+1,1)*dk^2)*w3(1)*exp(1i*w(N/2+1,1)*t);
Vy2(1) = sqrt(S(1,N/2+1)*dk^2)*w4(1)*exp(1i*w(1,N/2+1)*t);
Vx2e(1) = sqrt(Se(N/2+1,1)*dk^2)*w3(1)*exp(1i*we(N/2+1,1)*t);
Vy2e(1) = sqrt(Se(1,N/2+1)*dk^2)*w4(1)*exp(1i*we(1,N/2+1)*t);
for j = 2:N/2
    Vx2(j) = 1/2*sqrt(S(N/2+1,j)*dk^2)*(w3(j) + 1i*u3(j))*exp(1i*w(N/2+1,j)*t);
    Vy2(j) = 1/2*sqrt(S(j,N/2+1)*dk^2)*(w4(j) + 1i*u4(j))*exp(1i*w(j,N/2+1)*t);
    Vx2e(j) = 1/2*sqrt(Se(N/2+1,j)*dk^2)*(w3(j) + 1i*u3(j))*exp(1i*we(N/2+1,j)*t);
    Vy2e(j) = 1/2*sqrt(Se(j,N/2+1)*dk^2)*(w4(j) + 1i*u4(j))*exp(1i*we(j,N/2+1)*t);
end

Vx2(N/2+1) = sqrt(S(N/2+1,N/2+1)*dk^2)*u4(1)*exp(1i*w(N/2+1,N/2+1)*t);
Vy2(N/2+1) = sqrt(S(N/2+1,N/2+1)*dk^2)*u4(1)*exp(1i*w(N/2+1,N/2+1)*t);

Vx2e(N/2+1) = sqrt(Se(N/2+1,N/2+1)*dk^2)*u4(1)*exp(1i*we(N/2+1,N/2+1)*t);
Vy2e(N/2+1) = sqrt(Se(N/2+1,N/2+1)*dk^2)*u4(1)*exp(1i*we(N/2+1,N/2+1)*t);
